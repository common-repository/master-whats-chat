...
class Assets {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'tmwc-whatsapp-font', Functions::get_google_fonts_family( $settings ), array(), TMWC_VERSION );
		
		wp_register_style( 'tmwc-whatsapp-fontawesome-custom', TMWC_PLUGIN_URL . '/assets/vendor/tmw-fontawesome-custom/css/all.css', array(), TMWC_VERSION );
		wp_enqueue_style( 'tmwc-whatsapp-fontawesome-custom' );

		wp_register_style( 'tmwc-whatsapp-app', TMWC_PLUGIN_URL . '/assets/css/tmw-whatsapp-app.css', array(), TMWC_VERSION );
		...
		if( !empty($skinCSS) ) {
			wp_add_inline_style( 'tmwc-whatsapp-app', esc_html( $skinCSS ) );
		}

		wp_register_script( 'moment-timezone-with-data', TMWC_PLUGIN_URL . 'assets/vendor/moment/moment-timezone-with-data.min.js', array( 'moment' ), TMWC_VERSION, array( 'strategy' => 'defer', 'in_footer' => true ) );
		wp_enqueue_script( 'moment-timezone-with-data' );

		wp_register_script( 'tmwc-whatsapp-app', TMWC_PLUGIN_URL . '/assets/js/tmw-whatsapp-app.js', array( 'jquery' ), TMWC_VERSION, array( 'strategy' => 'defer', 'in_footer' => true ) );
		wp_localize_script( 'tmwc-whatsapp-app', 'tmwc_data', $data_to_localize );
		wp_enqueue_script( 'tmwc-whatsapp-app' );

		wp_register_script( 'tmwc-whatsapp-widgets', TMWC_PLUGIN_URL . '/assets/js/tmw-whatsapp-widgets.js', array( 'jquery' ), TMWC_VERSION, array( 'strategy' => 'defer', 'in_footer' => true ) );
	...
	public function print_styles() {
		...
		wp_register_script( 'tmwc-whatsapp-widgets', TMWC_PLUGIN_URL . '/js/tmw-whatsapp-widgets.js', array( 'jquery' ), TMWC_VERSION, array( 'strategy' => 'defer', 'in_footer' => true ) );
